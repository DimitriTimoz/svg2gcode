...
header="""(Generated by svg2gcode.py)
(Made by D. Timoz)
%{}

(Gravure)
""".format(randint(0,MAXINT))
target_size = (23*2, 23*2)
origin = (23,23)
...
def draw_circle(cx, cy, r):
    ...
    move_to_safe(sx,sy)
    move_to(sx,sy, depth)
    write("G3 X{:.2f} Y{:.2f} Z{:.2f} R{:.2f}".format(ox,oy, depth, r/1.995))
    write("G3 X{:.2f} Y{:.2f} Z{:.2f} R{:.2f}".format(sx,sy, depth, r/1.995))
...
def draw_arc(x1, y1, x2, y2, r, delta):
    ...
    move_to_safe(x1,y1)
    move_to(x1,y1, depth)
    write("G2 X{:.2f} Y{:.2f} Z{:.2f} R{:.2f}".format(x2,y2, depth, r))
    write("G0 Z{:.2f}".format( secZ))
    px, py, pz = x2, y2, secZ
